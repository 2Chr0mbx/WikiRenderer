...
public interface TextureDataProvider {
    ...
    default void buildTextureGrabSection(RenderScreen screen, FlowLayout layout) {
        ...
        IsometricUI.sectionHeader(layout, "player_textures", true);
        ...
        for (Map.Entry<String, MinecraftTexturesPayload> entry : foundTextures.entrySet()) {
            ...
            for (Map.Entry<MinecraftProfileTexture.Type, MinecraftProfileTexture> textureEntry : payload.textures().entrySet()) {
                ...
                try (IsometricUI.RowBuilder builder = IsometricUI.row(layout)) {
                    ...
                    builder.row.child(Components.label(mergedText).margins(Insets.of(0, 0, 0, 10)));

                    builder.row.child(Components.button(Translate.gui("open_url"), button -> Util.getPlatform().openUri(texture.getUrl())));
                    if (!GraphicsEnvironment.isHeadless()) {
                        builder.row.child(Components.button(Translate.gui("copy_texture_id"), button -> {
