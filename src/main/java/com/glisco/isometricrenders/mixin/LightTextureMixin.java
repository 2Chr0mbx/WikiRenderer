...
public class LightTextureMixin {
    ...
    private Object emulateDayLightColorForBlockRenders(EnvironmentAttributeProbe instance, EnvironmentAttribute<?> attribute, float f, Operation<Integer> original) {
        if (IsometricRenders.inRenderableDraw && AreaPropertyBundle.INSTANCE.emulateDaylight.get()) {
    ...
    private Object forceHighGamma(OptionInstance<Double> instance, Operation<Double> original) {
        if (IsometricRenders.inRenderableDraw && AreaPropertyBundle.INSTANCE.useFullBrightGamma.get()) {
            return 50D;
        }
    ...
    private boolean forceHasNightVision(boolean original) {
        ...
        if (IsometricRenders.inRenderableDraw && AreaPropertyBundle.INSTANCE.useNightVision.get()) {
            return true;
        }
        return original;
    }
