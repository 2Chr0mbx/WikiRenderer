...
public class MeshSection implements AutoCloseable {
    ...
    private VertexSorting orthographicSort;

    public MeshSection(SectionBufferBuilderPack bufferBuilderPack, Map<ChunkSectionLayer, MeshData> builtMeshes, VertexSorting orthographicSort) {
        this.bufferBuilderPack = bufferBuilderPack;
        this.builtMeshes = builtMeshes;
        this.orthographicSort = orthographicSort;
    }
    ...
    private ByteBuffer getSortedTranslucencyIndexBuffer(VertexSorting orthographicSort) {
        MeshData translucentMeshData = this.builtMeshes.get(ChunkSectionLayer.TRANSLUCENT);
        if (translucentMeshData == null) {
            return null;
        }

        MeshData.SortState transparencyState = translucentMeshData.sortQuads(bufferBuilderPack.buffer(ChunkSectionLayer.TRANSLUCENT), orthographicSort);
        if (transparencyState != null) {
            ByteBufferBuilder.Result result = transparencyState.buildSortedIndexBuffer(bufferBuilderPack.buffer(ChunkSectionLayer.TRANSLUCENT), orthographicSort);
            if (result != null) {
    ...
    public void upload() {
        builtMeshes.forEach((layer, meshData) -> {
            ...
            if (layer == ChunkSectionLayer.TRANSLUCENT && this.orthographicSort != null) {
                ByteBuffer sortedTranslucencyIndexBuffer = getSortedTranslucencyIndexBuffer(orthographicSort);
    ...
    public void reSortTransparencyData(@NotNull VertexSorting orthographicSort) {
        this.orthographicSort = orthographicSort;
        ByteBuffer sortedTranslucencyIndexBuffer = getSortedTranslucencyIndexBuffer(orthographicSort);
        if (sortedTranslucencyIndexBuffer != null) {
